...
export default function EventPage() {
  ...
  const [status, setStatus] = useState<Record<number, TaskStatus>>({ 1: 'idle', 2: 'idle', 3: 'idle' });
  const [startTimes, setStartTimes] = useState<Record<number, number | null>>({ 1: null, 2: null, 3: null });
  ...
  useEffect(() => {
    if (isAllDone && !completionTime) {
      const now = new Date();
      setCompletionTime(now.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }) + " - " + now.toLocaleDateString('vi-VN'));
    }
  ...
  const handleVerify = (id: number) => {
    ...
    if (elapsed < 15) {
      ...
      alert("⚠️ Vui lòng thực hiện lại nhiệm vụ");
    } else {
  ...
  return (
    <main className="bg-[#0a0a0c] min-h-screen text-white pb-32 overflow-x-hidden">
      ...
      <section className="relative h-[70vh] md:h-[75vh] w-full flex flex-col items-center justify-center">
        <img src="/assets/event-banner.png" className="absolute inset-0 w-full h-full object-cover opacity-30" alt="Banner" />
      ...
      <section className="max-w-4xl mx-auto px-6 py-20 md:py-32 text-gray-500 italic leading-relaxed text-sm md:text-lg">
        <p className="mb-6 border-l-4 border-yellow-500 pl-4 md:pl-6 text-white font-bold italic">
          "Thành công chỉ đến với người kiên nhẫn. Hoàn thành nhiệm vụ để mở khóa rương báu Overlix."
        </p>
      ...
      <div className="sticky top-20 md:top-28 z-40 max-w-xl mx-auto px-4 md:px-6 mb-12 md:mb-16">
        <div className="bg-black/80 backdrop-blur-xl p-4 md:p-5 rounded-2xl md:rounded-3xl border border-white/10 shadow-2xl">
          <div className="flex justify-between text-[10px] md:text-xs font-black uppercase mb-2 md:mb-3 italic">
            <span className="flex items-center gap-2 text-yellow-500"><Sparkles size={14} /> Tiến độ</span>
            <span>{completedTasks.length} / {totalTasks}</span>
          </div>
          <div className="h-2 md:h-2.5 w-full bg-white/5 rounded-full overflow-hidden">
            <motion.div animate={{ width: `${progress}%` }} className="h-full bg-yellow-500 shadow-[0_0_20px_rgba(234,179,8,0.6)]" />
          </div>
        </div>
      </div>

      {/* SECTION 4: MISSIONS LIST */}
      <section ref={missionRef} className="max-w-4xl mx-auto px-4 md:px-6 py-10 relative">
        <div className={`space-y-3 md:space-y-4 transition-all duration-1000 ${isAllDone ? "blur-xl opacity-20 pointer-events-none scale-95" : ""}`}>
          {TASKS_CONFIG.map((task) => (
            <div key={task.id} className={`p-4 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border transition-all duration-500 flex flex-col md:flex-row md:items-center md:justify-between gap-4 ${
              status[task.id] === 'done' ? 'bg-green-500/5 border-green-500/40' : 'bg-[#111114] border-white/5'
            }`}>
              
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 md:w-14 md:h-14 shrink-0 rounded-xl flex items-center justify-center font-black text-sm md:text-xl ${
                  status[task.id] === 'done' ? 'bg-green-500 text-black rotate-[360deg] transition-transform duration-700' : 'bg-white/5 text-gray-500'
                }`}>
                  {status[task.id] === 'done' ? <CheckCircle2 size={20} /> : task.id}
                </div>
                <div className="flex-1">
                  <h3 className="font-black text-base md:text-xl uppercase italic tracking-tight leading-none mb-1">{task.title}</h3>
                  <p className="text-gray-500 text-[10px] md:text-sm italic">{task.desc}</p>
                </div>
              </div>

              <div className="flex justify-end md:block relative">
                {status[task.id] === 'idle' && (
                  <button onClick={() => handleStart(task.id, task.url)} className="px-5 py-2 md:px-8 md:py-3 bg-white text-black font-black rounded-xl uppercase italic text-[10px] md:text-sm shadow-md active:scale-95 transition-all">
                    Làm ngay
                  </button>
                )}
                {status[task.id] === 'checking' && (
                  <button onClick={() => handleVerify(task.id)} className="px-5 py-2 md:px-8 md:py-3 bg-yellow-500 text-black font-black rounded-xl uppercase italic text-[10px] md:text-sm animate-pulse shadow-lg">
                    Kiểm tra
                  </button>
                )}
                {status[task.id] === 'done' && (
          ...
          {isAllDone && (
            ...
            >
              <div className="bg-yellow-500 text-black p-8 md:p-12 rounded-[2.5rem] md:rounded-[3rem] shadow-[0_0_80px_rgba(234,179,8,0.7)] text-center w-full max-w-lg border-[4px] md:border-[6px] border-black relative">
                ...
                <h2 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter mb-2 leading-none">Victory!</h2>
                <p className="font-bold text-xs md:text-base mb-6 md:mb-8 uppercase opacity-90">Sứ mệnh hoàn tất</p>
                ...
                <div className="bg-black/10 rounded-2xl p-4 md:p-5 mb-8 space-y-3 font-black italic text-xs md:text-sm text-left">
                  <div className="flex justify-between items-center border-b border-black/10 pb-2">
                    <span className="flex items-center gap-2"><User size={16} /> Username:</span>
